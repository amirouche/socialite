...
def extract(raw, source):
    ...
    root = xml.tag
    if root == "rss":
        out = {"title": xml.xpath("/rss/channel/title/text()")[0], "entries": []}
        ...
        title = xml.xpath("/atom:feed/atom:title/text()", namespaces=namespaces)[
            0
        ].strip()
        out = {"source": source, "title": title, "entries": []}
...
async def add(tr, sparky, feed):
    ...
    tuples = [(uid, "name", feed["title"]), (uid, "is a", "feed")]
